  ...
  return (
    <Card className="m-4 h-60">
      <CardBody>
        ...
        <div className="text-sm text-default-500 m-2">
          {status === 2 ? 'Ready to encrypt transaction.' : status === 3 ? (<span className="flex gap-3"><Spinner size="sm" /> Encrypting transaction...</span>) : status > 3 && 'Transaction encrypted.'}
        </div>
        <div className="text-sm text-default-500 m-2">
          {status === 4 ? 'Ready to submit transaction.' : status === 5 ? (<span className="flex gap-3"><Spinner size="sm" /> Submitting transaction...</span>) : status > 5 && 'Transaction submitted.'}
        </div>
